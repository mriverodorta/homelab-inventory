...
const dockerfile = await fs.readFile(path.join(root, 'docker', 'agent-artifact.Dockerfile'), 'utf8')
...
if (!dockerfile.includes(`ARG AGENT_VERSION=${pin.version}`)) throw new Error('Agent artifact Dockerfile version does not match the application pin.')
if (!dockerfile.includes(`ARG AGENT_SOURCE_REVISION=${pin.sourceRevision}`)) throw new Error('Agent artifact Dockerfile source revision does not match the application pin.')
