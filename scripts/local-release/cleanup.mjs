...
export function releaseCleanupCommands({ revision, candidateArchitectures = [] } = {}) {
  const commands = [
    ['docker', 'buildx', 'rm', '--force', RELEASE_BUILDER],
    ['docker', 'scout', 'cache', 'prune', '--force', '--sboms'],
    ['docker', 'volume', 'rm', '--force', RELEASE_TRIVY_CACHE_VOLUME],
  ]
...
export async function cleanupReleaseDockerState({
  ...
  for (const command of releaseCleanupCommands({ revision, candidateArchitectures })) {
    await run(command, { allowFailure: true, capture: true, log: false })
  }
  await removeOrphanedReleaseRegistries()
  await run(['docker', 'image', 'rm', '--force', TRIVY_IMAGE, LOCAL_REGISTRY_IMAGE, RELEASE_BUILDKIT_IMAGE], {
    allowFailure: true,
    capture: true,
    log: false,
  })
