...
export async function createRemoteSnapshot(config, paths, { root } = {}) {
  ...
  try {
    ...
    const command = [
      ...
      `docker exec "$cid" bun -e 'const fs=await import("node:fs/promises"); await fs.rm("/tmp/release-snapshot",{recursive:true,force:true}); await fs.rm("/tmp/${REMOTE_HELPER}",{force:true}).catch(()=>{})'`,
