...
describe('business-data synchronization sharing boundary', () => {
  ...
  it('keeps the destination credential and recovery hashes stable across repeated synchronization', async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), 'hli-sharing-sync-restart-'))
    roots.push(root)
    const source = path.join(root, 'source')
    const destination = path.join(root, 'destination')
    const first = path.join(root, 'first')
    const second = path.join(root, 'second')
    await Promise.all([fs.mkdir(path.join(source, 'stores'), { recursive: true }), fs.mkdir(path.join(destination, 'stores'), { recursive: true }), fs.mkdir(path.join(destination, 'sharing'), { recursive: true })])
    for (const directory of [source, destination]) for (const name of ['inventory.json', 'project.json']) await fs.writeFile(path.join(directory, 'stores', name), '{}')
    for (const name of identityNames) await fs.writeFile(path.join(destination, 'sharing', name), `destination-${name}`)
    const expected = await identityHashes(destination)
    await execFileAsync('bash', [path.resolve('scripts/synchronize-business-data.sh'), source, destination, first])
    await execFileAsync('bash', [path.resolve('scripts/synchronize-business-data.sh'), source, first, second])
    expect(await identityHashes(first)).toEqual(expected)
    expect(await identityHashes(second)).toEqual(expected)
  })
})
