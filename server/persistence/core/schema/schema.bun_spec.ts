...
describe('core SQLite foundation schema', () => {
  test('exports the typed foundation tables', () => {
    ...
    expect(CORE_MIGRATIONS).toHaveLength(24)
    expect(CORE_MIGRATIONS.at(-1)?.id).toBe('0024_m2_ae_projection_repair')
  })
  ...
  test('migrates network and wireless records into one relational adapter model', async () => {
    ...
    try {
      ...
      await expect(applyCommittedMigrations(handle, migrations)).resolves.toEqual({ applied: 7, currentVersion: 24 })
      ...
      `).all()).toEqual([])
      await expect(applyCommittedMigrations(handle, migrations)).resolves.toEqual({ applied: 0, currentVersion: 24 })
    } finally {
  ...
  test('repairs unambiguous missing M.2 metadata without overwriting or guessing', async () => {
    ...
    try {
      ...
      await expect(applyCommittedMigrations(handle, migrations)).resolves.toEqual({ applied: 6, currentVersion: 24 })
  ...
  test('reclassifies an unambiguous M.2 A/E expansion resource without changing relational IDs', async () => {
    ...
    try {
      ...
      await expect(applyCommittedMigrations(handle, migrations)).resolves.toEqual({ applied: 2, currentVersion: 24 })
  ...
  test('migrates v10 NAS topology without changing existing relational identities', async () => {
    ...
    try {
      ...
      await expect(applyCommittedMigrations(handle, migrations)).resolves.toEqual({ applied: 10, currentVersion: 24 })
      expect(handle.database.query(`
