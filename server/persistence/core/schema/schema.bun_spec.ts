...
describe('core SQLite foundation schema', () => {
  test('exports the typed foundation tables', () => {
    ...
    expect(CORE_MIGRATIONS).toHaveLength(34)
    expect(CORE_MIGRATIONS.at(-1)?.id).toBe('0034_sharing_event_lifecycle_hardening')
  })
  ...
  test('migrates network and wireless records into one relational adapter model', async () => {
    ...
    try {
      ...
      await expect(applyCommittedMigrations(handle, migrations)).resolves.toEqual({ applied: 17, currentVersion: 34 })
      ...
      `).all()).toEqual([])
      await expect(applyCommittedMigrations(handle, migrations)).resolves.toEqual({ applied: 0, currentVersion: 34 })
    } finally {
  ...
  test('repairs unambiguous missing M.2 metadata without overwriting or guessing', async () => {
    ...
    try {
      ...
      await expect(applyCommittedMigrations(handle, migrations)).resolves.toEqual({ applied: 16, currentVersion: 34 })
  ...
  test('reclassifies an unambiguous M.2 A/E expansion resource without changing relational IDs', async () => {
    ...
    try {
      ...
      await expect(applyCommittedMigrations(handle, migrations)).resolves.toEqual({ applied: 12, currentVersion: 34 })
  ...
  test('migrates v10 NAS topology without changing existing relational identities', async () => {
    ...
    try {
      ...
      await expect(applyCommittedMigrations(handle, migrations)).resolves.toEqual({ applied: 20, currentVersion: 34 })
      expect(handle.database.query(`
