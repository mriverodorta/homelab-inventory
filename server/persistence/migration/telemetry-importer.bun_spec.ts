...
describe('telemetry reference migration', () => {
  test('rekeys a staged clone and preserves source samples exactly', async () => {
    ...
    try {
      ...
      expect(migrated.query('SELECT agent_id, host_item_id, sequence, received_at_ms, collected_at_ms FROM heartbeat_receipts').get())
        .toEqual({ agent_id: 13, host_item_id: 41, sequence: 4, received_at_ms: 1001, collected_at_ms: 1000 })
      expect(migrated.query('SELECT host_item_id, cpu_percent, memory_used_bytes FROM host_metric_samples').get())
        .toEqual({ host_item_id: 41, cpu_percent: 12.5, memory_used_bytes: 1024 })
      expect(migrated.query('SELECT host_item_id, uptime_seconds, memory_json FROM host_runtime_state').get())
        .toEqual({ host_item_id: 41, uptime_seconds: 300, memory_json: '{"availableBytes":null,"buffersBytes":null,"cachedBytes":null,"swapTotalBytes":null,"swapUsedBytes":null,"totalBytes":4096}' })
      expect(migrated.query("SELECT count(*) AS count FROM sqlite_master WHERE name IN ('telemetry_samples', 'network_interface_samples', 'storage_device_samples', 'filesystem_samples')").get())
        .toEqual({ count: 0 })
    } finally {
  ...
  test('migrates telemetry histories larger than one processing batch', async () => {
    ...
    const migrated = new Database(targetPath, { readonly: true, strict: true })
    expect(migrated.query('SELECT count(*) AS count FROM heartbeat_receipts WHERE agent_id = 13 AND host_item_id = 41').get())
      .toEqual({ count: 30 })
    expect(migrated.query('SELECT count(*) AS count FROM host_metric_samples').get()).toEqual({ count: 1 })
    expect(migrated.query('SELECT count(*) AS count FROM component_events WHERE host_item_id = 41').get()).toEqual({ count: 0 })
    migrated.close(false)
  })
