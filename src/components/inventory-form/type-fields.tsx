...
export function InventoryTypeFields({
  ...
  if (type === 'cpu') {
    return (
      <div className="grid gap-3 sm:grid-cols-3">
        <TextField label="Family" name="family" value={values.family} placeholder="Core i5" onChange={(family) => onChange({ family })} />
        <TextField label="Number" name="number" value={values.number} placeholder="i5-10500T" onChange={(number) => onChange({ number })} />
        <TextField label="Cores" name="cores" value={values.cores} type="number" min={1} error={errors.cores} onChange={(cores) => onChange({ cores })} />
        <TextField label="Threads" name="threads" value={values.threads} type="number" min={1} error={errors.threads} onChange={(threads) => onChange({ threads })} />
        <TextField label="Base Clock" name="baseClockGhz" value={values.baseClockGhz} type="number" min={0} step="0.1" error={errors.baseClockGhz} onChange={(baseClockGhz) => onChange({ baseClockGhz })} />
        <TextField label="Boost Clock" name="boostClockGhz" value={values.boostClockGhz} type="number" min={0} step="0.1" error={errors.boostClockGhz} onChange={(boostClockGhz) => onChange({ boostClockGhz })} />
      </div>
    )
  }
  ...
  if (type === 'ram') {
    return (
      <div className="grid gap-3 sm:grid-cols-3">
        <TextField label="Capacity GB" name="capacityGb" value={values.capacityGb} type="number" min={1} error={errors.capacityGb} onChange={(capacityGb) => onChange({ capacityGb })} />
        ...
        <TextField label="Stick 2 Manufacturer" name="secondaryManufacturer" value={values.secondaryManufacturer} placeholder="Same as stick 1" onChange={(secondaryManufacturer) => onChange({ secondaryManufacturer })} />
  ...
  if (type === 'storage') {
    return (
      <div className="grid gap-3 sm:grid-cols-4">
        <TextField className="sm:col-span-2" label="Capacity" name="capacity" value={values.capacity} type="number" min={0} step="0.1" error={errors.capacity} onChange={(capacity) => onChange({ capacity })} />
  ...
  if (type === 'nas') {
    return (
      <div className="grid gap-3 sm:grid-cols-3">
        <TextField label="Drive Bays" name="driveBays" value={values.driveBays} type="number" min={0} error={errors.driveBays} onChange={(driveBays) => onChange({ driveBays })} />
        <TextField label="M.2 Slots" name="m2Slots" value={values.m2Slots} type="number" min={0} error={errors.m2Slots} onChange={(m2Slots) => onChange({ m2Slots })} />
      </div>
    )
  }
  ...
  if (type === 'gpu') {
    return (
      <div className="grid gap-3 sm:grid-cols-4">
        <TextField label="VRAM GB" name="vramGb" value={values.vramGb} type="number" min={0} error={errors.vramGb} onChange={(vramGb) => onChange({ vramGb })} />
  ...
  if (type === 'switch') {
    return (
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <TextField label="Switching Gbps" name="switchingCapacityGbps" value={values.switchingCapacityGbps} type="number" min={0} error={errors.switchingCapacityGbps} onChange={(switchingCapacityGbps) => onChange({ switchingCapacityGbps })} />
  ...
  if (type === 'patchPanel') {
    return (
      <div className="grid gap-3 sm:grid-cols-2">
        <TextField label="Rack Units" name="rackUnits" value={values.rackUnits} type="number" min={0} step="0.5" error={errors.rackUnits} placeholder="1" onChange={(rackUnits) => onChange({ rackUnits })} />
        <TextField label="Mount" name="mount" value={values.mount} placeholder="Rack mounted" onChange={(mount) => onChange({ mount })} />
      </div>
    )
  }
