...
function CreateBackupDialog({ open, onOpenChange, sections, definitions, onCreate, busy }: { open: boolean; onOpenChange: (open: boolean) => void; sections: BackupSectionName[]; definitions: Definitions; onCreate: (input: { label: string; sections: BackupSectionName[]; encryptStoredCopy: boolean; passphrase?: string }) => Promise<void>; busy: boolean }) {
  ...
  return <Dialog open={open} onOpenChange={busy ? undefined : onOpenChange}><DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-2xl">
    ...
    <div className="flex items-center justify-between gap-4 rounded-md border border-[#ded8ce] bg-[#f8f4ed] p-3"><div><p className="text-sm font-black">Encrypt stored copy</p><p className="mt-1 text-xs leading-5 text-[#756d62]">{authenticationSelected ? 'Required because authentication and access data is included.' : 'Use AES-256-GCM with a passphrase you must retain.'}</p></div><Switch checked={effectiveEncrypted} disabled={authenticationSelected} onCheckedChange={setEncrypted} /></div>
...
export function BackupRestoreSettings() {
  ...
  const canDownload = usePermission('backups.download')
  const canDelete = usePermission('backups.delete')
  const canSchedule = usePermission('backups.schedule')
  ...
  if (status.mode === 'demo') return <SettingsSection title="Backup & Restore" description="Public demo sessions can export only their disposable sandbox data."><SettingRow label="Export demo sandbox" description="Downloads inventory and project data without credentials, agents, or server-side backup storage.">{canDownload ? <Button onClick={() => void downloadDemoBackup()}><Download />Download sandbox</Button> : null}</SettingRow></SettingsSection>
  ...
  return <SettingsSection title="Backup & Restore" description="Create portable backups, restore selected sections, and manage automatic retention."><Tabs defaultValue="backups" className="gap-0"><TabsList variant="line" className="mx-4 mt-3"><TabsTrigger value="backups">Backups</TabsTrigger>{canSchedule ? <TabsTrigger value="schedule">Schedule</TabsTrigger> : null}</TabsList>
    <TabsContent value="backups" className="grid gap-0 pt-3"><div className="flex flex-wrap gap-2 border-y border-[#e8e1d6] p-4">{canCreate ? <Button onClick={() => setCreateOpen(true)}><Archive />Create backup</Button> : null}{canRestore ? <Button variant="outline" onClick={() => setRestoreOpen(true)}><FileUp />Restore from file</Button> : null}<span className="ml-auto self-center text-xs font-semibold text-[#756d62]">{status.backups.length} stored · {bytes(status.storageBytes ?? 0)}</span></div>{status.operation ? <div className="flex items-center gap-2 border-b border-[#e8e1d6] bg-[#f7f2e9] p-4 text-sm font-bold"><LoaderCircle className="size-4 animate-spin" />{status.operation.kind} in progress</div> : null}<div className="grid gap-3 p-4">{ordinary.map((backup) => <div key={backup.id} className="grid gap-3 rounded-md border border-[#ded8ce] p-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"><div className="min-w-0"><p className="truncate text-sm font-black">{backup.label}</p><p className="mt-1 text-xs text-[#756d62]">{new Date(backup.createdAt).toLocaleString()} · {bytes(backup.sizeBytes)} · {backup.sections.length} sections{backup.encrypted ? ' · encrypted' : ''}</p></div><div className="flex flex-wrap gap-1.5"><Button size="sm" variant="outline" onClick={() => backup.encrypted ? setCredentialAction({ backup, action: 'verify' }) : void backups.verify.mutateAsync({ id: backup.id }).then(() => setMessage('Backup verification passed.')).catch((error) => setMessage(error.message))}><CheckCircle2 />Verify</Button>{canDownload ? <Button size="sm" variant="outline" onClick={() => setCredentialAction({ backup, action: 'download' })}><Download />Download</Button> : null}{canDelete ? <Button size="icon-sm" variant="ghost" title="Delete backup" onClick={() => setDeleteRecord(backup)}><Trash2 /></Button> : null}</div></div>)}{ordinary.length === 0 ? <div className="rounded-md border border-dashed border-[#cfc6b9] p-6 text-center text-sm text-[#756d62]">No portable backups have been stored yet.</div> : null}{recovery.length > 0 ? <details className="rounded-md border border-[#ded8ce] p-3"><summary className="cursor-pointer text-sm font-black">Recovery backups ({recovery.length})</summary><div className="mt-3 grid gap-2">{recovery.map((backup) => <div key={backup.id} className="flex items-center justify-between gap-3 text-xs"><span>{backup.label} · {new Date(backup.createdAt).toLocaleString()}</span>{canDownload ? <Button size="sm" variant="outline" onClick={() => setCredentialAction({ backup, action: 'download' })}>Download</Button> : null}</div>)}</div></details> : null}{status.restores.length > 0 ? <details className="rounded-md border border-[#ded8ce] p-3"><summary className="cursor-pointer text-sm font-black">Restore history ({status.restores.length})</summary><div className="mt-3 grid gap-2">{status.restores.map((restore) => <p key={restore.id} className="text-xs text-[#665d52]">#{restore.id} {restore.status} · {new Date(restore.startedAt).toLocaleString()} · {restore.sections.length} sections</p>)}</div></details> : null}</div></TabsContent>
    {canSchedule ? <ScheduleTab key={schedule.updatedAt ?? 'initial'} schedule={schedule} environment={status.environment} busy={backups.schedule.isPending} onSave={saveSchedule} /> : null}
  </Tabs>{message ? <p role="status" className="border-t border-[#e8e1d6] px-4 py-3 text-xs font-semibold text-[#665d52]">{message}</p> : null}
    {canCreate ? <CreateBackupDialog key={String(createOpen)} open={createOpen} onOpenChange={setCreateOpen} sections={sections} definitions={definitions} busy={backups.create.isPending} onCreate={async (input) => { await backups.create.mutateAsync(input); setMessage('Backup created and verified.') }} /> : null}
    ...
    {canRestore ? <RestoreDialog key={String(restoreOpen)} open={restoreOpen} onOpenChange={setRestoreOpen} definitions={definitions} inspect={(file, passphrase) => backups.inspect.mutateAsync({ file, passphrase })} preflight={(token, selected) => backups.preflight.mutateAsync({ token, sections: selected })} restore={async (token, selected) => { await backups.restore.mutateAsync({ token, sections: selected }) }} /> : null}
    {canDelete ? <AlertDialog open={deleteRecord !== null} onOpenChange={(open) => { if (!open) setDeleteRecord(null) }}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Delete stored backup?</AlertDialogTitle><AlertDialogDescription>{deleteRecord?.label} will be removed permanently. This does not change inventory or project data.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction variant="destructive" onClick={() => { if (deleteRecord) void backups.remove.mutateAsync(deleteRecord.id).then(() => setDeleteRecord(null)) }}>Delete backup</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog> : null}
  </SettingsSection>
}
