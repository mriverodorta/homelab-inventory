...
function CreateBackupDialog({ open, onOpenChange, sections, definitions, onCreate, busy }: { open: boolean; onOpenChange: (open: boolean) => void; sections: BackupSectionName[]; definitions: Definitions; onCreate: (input: { label: string; sections: BackupSectionName[]; encryptStoredCopy: boolean; passphrase?: string }) => Promise<void>; busy: boolean }) {
  ...
  async function submit() {
    setError(null)
    try { await onCreate({ label, sections: effective, encryptStoredCopy: encrypted, passphrase: encrypted ? passphrase : undefined }); onOpenChange(false) }
  ...
  return <Dialog open={open} onOpenChange={busy ? undefined : onOpenChange}><DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-2xl">
    ...
    <div className="flex items-center justify-between gap-4 rounded-md border border-[#ded8ce] bg-[#f8f4ed] p-3"><div><p className="text-sm font-black">Encrypt stored copy</p><p className="mt-1 text-xs leading-5 text-[#756d62]">Use AES-256-GCM with a passphrase you must retain.</p></div><Switch checked={encrypted} onCheckedChange={setEncrypted} /></div>
    {encrypted ? <label className="grid gap-1.5 text-sm font-bold">Passphrase<Input type="password" autoComplete="new-password" minLength={12} value={passphrase} onChange={(event) => setPassphrase(event.target.value)} /><span className="text-xs font-normal text-[#756d62]">Use at least 12 characters.</span></label> : null}
    {error ? <p role="alert" className="text-sm font-semibold text-[#9b3f32]">{error}</p> : null}
    <DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)} disabled={busy}>Cancel</Button><Button onClick={() => void submit()} disabled={busy || effective.length === 0 || !label.trim() || (encrypted && passphrase.length < 12)}>{busy ? <LoaderCircle className="animate-spin" /> : <Archive />}Create backup</Button></DialogFooter>
...
function BackupCredentialDialog({ backup, action, open, onOpenChange, onSubmit, busy }: { backup: BackupRecord | null; action: 'download' | 'verify'; open: boolean; onOpenChange: (open: boolean) => void; onSubmit: (passphrase: string) => Promise<void>; busy: boolean }) {
  ...
  const needsPassphrase = backup.encrypted || (action === 'download' && backup.sections.some((section) => ['registryEnrollment', 'agents', 'agentTelemetry'].includes(section)))
