  ...
  return (
    <div className="grid gap-6">
      ...
      {settings.connectionEnabled ? <SettingsSection title="Shares" description="Each share has its own views, privacy selections, visibility, expiration, and update policy.">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#e8e1d6] p-4"><div className="flex items-center gap-2 text-sm text-[#554b40]"><Cloud className="size-4" />{sharing.shares.data?.length ?? 0} configured</div>{canPublish ? <div className="flex gap-2"><Button variant="outline" onClick={() => setClaimOpen(true)}><UserRoundPlus />Connect account</Button><Button onClick={() => void openEditor()} disabled={!workbooks.data?.length}><Plus />New share</Button></div> : null}</div>
      ...
      {settings.connectionEnabled && (sharing.shares.data?.length ?? 0) > 0 ? <SettingsSection title="Audience" description="Owner analytics are aggregated and never expose raw request records."><ShareAnalytics /></SettingsSection> : null}

      <ShareDialog open={editorOpen} configuration={editing} workbooks={workbooks.data ?? []} metadata={metadata.data ?? null} busy={sharing.create.isPending || sharing.update.isPending} onOpenChange={setEditorOpen} onSave={saveShare} />
      ...
      <AccountClaimDialog open={claimOpen} origin={status.origin} pending={sharing.claim.isPending} result={claimResult} error={sharing.claim.error instanceof Error ? sharing.claim.error.message : null} onOpenChange={(open) => { setClaimOpen(open); if (!open) setClaimResult(null) }} onBegin={() => { void sharing.claim.mutateAsync().then(setClaimResult).catch(() => undefined) }} />
    </div>
  )
}
