...
- A translation conflict or invalid canonical value fails catalog activation instead of silently dropping a facet value. Historical legacy fields whose units are genuinely ambiguous or inexact, such as old `cacheMb` values without binary-unit evidence or textual power ranges, remain unchanged while exact canonical measurements are still projected.
